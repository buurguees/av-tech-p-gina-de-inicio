...
const RegisterPurchasePaymentDialog = ({
  ...
  const isNegativeInvoice =
    (totalAmount !== undefined ? totalAmount : pendingAmount) < 0;

  const [amount, setAmount] = useState(
    payment
      ? Math.abs(payment.amount).toString()
      : Math.abs(pendingAmount).toString(),
  );
  ...
  const fetchCreditProviders = async () => {
    try {
      ...
      if (error) throw error;
      setCreditProviders(
        ((data as unknown as CreditProvider[]) || []).filter(
          (p) => p.is_active,
        ),
      );
    } catch (err) {
  ...
  const maxAllowed = isEditing
    ? absPendingAmount + absPaymentAmount
    : absPendingAmount;
      ...
      if (paymentMode === "PERSONAL") {
        ...
        if (!selectedPartnerId) {
          toast({
            variant: "destructive",
            title: "Error",
            description: "Selecciona el socio que realizó el pago",
          });
          setLoading(false);
          return;
        }
        ...
        toast({
          ...
          description: `Registrado como deuda con socio. Pendiente de reembolso.`,
        });
        ...
        if (!selectedProviderId) {
          toast({
            variant: "destructive",
            title: "Error",
            description: "Selecciona el proveedor de financiación",
          });
          setLoading(false);
          return;
        }
        ...
        const { error } = await supabase.rpc("create_credit_operation", {
          ...
          p_settlement_bank_account_id:
            bankAccountId === "NONE" ? null : bankAccountId,
        });
        if (error) throw error;
        toast({
          title: "Financiación confirmada",
          description: `Deuda reclasificada a acreedor financiero. ${installments} cuota(s) generada(s).`,
        });
      } else {
        // Standard payment
        if (isEditing) {
  ...
  const accentGradientClass = {
    amber: "bg-gradient-to-br from-amber-500/10 via-transparent to-transparent",
    blue: "bg-gradient-to-br from-blue-500/10 via-transparent to-blue-500/5",
    emerald:
      "bg-gradient-to-br from-emerald-500/10 via-transparent to-transparent",
    red: "bg-gradient-to-br from-red-500/10 via-transparent to-blue-500/5",
  }[color];
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-h-[90vh] max-w-lg overflow-y-auto rounded-[2.25rem] border-border bg-card p-0 text-foreground shadow-xl">
        <div
          className={cn(
        ...
        <div className="p-6 md:p-8 space-y-5">
          <DialogHeader className="space-y-2">
            <div className="flex items-center gap-3">
              <div
                className={cn("p-2.5 rounded-2xl shadow-inner", {
                  "bg-amber-500/10 border border-amber-500/20":
                    isEditing || paymentMode === "PERSONAL",
                  "bg-blue-500/10 border border-blue-500/20":
                    paymentMode === "APLAZAME",
                  "bg-emerald-500/10 border border-emerald-500/20":
                    isNegativeInvoice && paymentMode === "STANDARD",
            ...
            <DialogDescription className="text-muted-foreground text-sm">
              {paymentMode === "PERSONAL"
                ? "El socio pagó con medios personales. Se registrará como deuda pendiente de reembolso."
                : paymentMode === "APLAZAME"
                  ? "La deuda se reclasifica del proveedor al acreedor financiero."
                  : isEditing
          ...
          {!isEditing && !isNegativeInvoice && (
            <div className="rounded-3xl border border-border/70 bg-muted/35 p-2 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className={fieldPanelClass}>
                ...
                <Input
                  ...
                  className={cn("font-mono text-lg", fieldControlClass)}
                  placeholder="0.00"
                />
              </div>
              {paymentMode === "STANDARD" && (
              ...
              <div className={fieldPanelClass}>
                <Label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">
                  {paymentMode === "APLAZAME"
                    ...
                    : isNegativeInvoice
                      ? "Recibido en (Cuenta Banco)"
                      : "Pagado desde (Cuenta Banco)"}
                </Label>
