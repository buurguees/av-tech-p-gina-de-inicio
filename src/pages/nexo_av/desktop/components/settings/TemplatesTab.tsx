...
import { FileText, Receipt, Eye, Loader2, AlertCircle } from "lucide-react";
import {
  Document,
  Page,
  Text,
...
export function TemplatesTab() {
  ...
  const fetchFirstInvoice = async () => {
    try {
      ...
      // Get all invoices - finance_list_invoices doesn't support pagination
      ...
      // Get first invoice (most recent)
      const firstInvoice = invoicesData[0];
      setInvoice(firstInvoice);

      // Fetch invoice details using finance_get_invoice
      const { data: invoiceDetailData, error: invoiceDetailError } = await supabase.rpc(
      ...
      const invoiceRecord = Array.isArray(invoiceDetailData) ? invoiceDetailData[0] : invoiceDetailData;
      if (invoiceRecord) {
                  ...
                  <span className="text-white/60 text-sm">
                    {invoiceLoading 
                      ...
                      : noInvoices 
                        ...
                        : `Vista previa - Plantilla de Factura (${invoice?.invoice_number || invoice?.preliminary_number || 'N/A'})`}
                  </span>
                </div>
                {!noInvoices && (
                  <div className="flex items-center gap-2">
